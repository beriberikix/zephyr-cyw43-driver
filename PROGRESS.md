...
| SOAK | Coexistence soak passes (STA assoc + BLE connected + bidirectional load; zero lockups/faults/disconnects) | VERIFIED (bounded load — operator-chosen accept+document path) | test/coex/results/soak_bounded_8boot_20260619.log — STA assoc + BLE connected + notify ~9.6/s + bounded bidirectional WiFi (dev ping 1/s + host ping 0.5/s): 8 SWD-reset cold boots, 0 faults, 0 unexpected disconnects, 6070 notif / 630s. + soak_bounded_long_20260619.log (sustained single connection). The heavy-load gSPI-corruption fault (root-caused below the driver in the vendored cybt HAL; persists with the bus lock held) is OUT of the certified envelope and documented in REFERENCE.md §2.7 with a transport-level fix recommendation. Arbitration hardened: poll coop -10 (5c17ff6) + BT-TX bus lock (8df7566). See "SOAK ROOT CAUSE — gSPI corruption". |
