...
| W5 | Init/power-order matrix (BT-only / WiFi→BT / BT→WiFi) + WiFi-only WHD build | IN-PROGRESS | BT-only ordering PASS (bt init -> BD_ADDR 88:A2:9E:D1:6D:A0 verified, advertising, ALIVE; WiFi-only WHD build green). BUT WiFi→BT ordering FAULTS — a corrupt-stack/PC memory fault (z_main_stack, PC in RAM) under WiFi+BT concurrency, distinct from the cybt assert. Same deeper coex issue blocking W4's BLE link. Needs the coex memory-fault root-cause before W5 can complete. |
