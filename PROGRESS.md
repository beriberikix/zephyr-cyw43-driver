...
| W6 | §2.7 fix: BT ring-index read via WHD F1-overflow-aware path (durable hal_rpi_pico patch); full 2 h soak zero faults | **VERIFIED** | docs/artifacts/w6_soak_2h_clean_20260620.log — **2 h CONTINUOUS WiFi+BLE coexistence under moderate load: 71021 notifications / 7200.1 s @ 9.9/s, 0 stalls, 0 unexpected disconnects, 0 faults (ALIVE). THE DONE GATE — met where georgerobotics could not (§2.7 fault at ~19 min).** Reached via FOUR fixes: (1) shared gSPI bus lock (patches/airoc_whd_hal_spi_shared_bus_lock.patch); (2) cybt re-read-not-assert (patches/cybt_shared_bus_reread_index.patch); (3) BT-coex NVRAM btc_mode=1 (patches/whd_nvram_43439_1yn_btcoex.patch) for BT TX power; (4) BT-first buffer arbitration (patches/airoc_wifi_bt_backplane_buffer_reserve.patch) for the sustained-load TX-queue stall. PRIOR (now-superseded) note: docs/artifacts/w4_whd_coex_characterization_20260619.log — shared gSPI bus lock LANDED (patches/airoc_whd_hal_spi_shared_bus_lock.patch wraps whd_bus_spi_transfer; whd_transport.c defines whd_bus_lock; whd_bt_glue.c BT path takes it). Two §2.7 mitigations now in place: (1) cybt reads route through WHD's F1-overflow-aware backplane path → no more assert/panic (0 faults observed); (2) shared lock serializes WLAN vs BT → WiFi associates with BT active. PART 2 LANDED (the cybt re-read, the §2.7 transport fix proper): patches/cybt_shared_bus_reread_index.patch makes cybt_get_bt_buf_index RE-READ a transiently out-of-range ring index (up to 8×) and return CYBT_ERR_HCI_READ_FAILED instead of assert()/abort() — fixed a real boot-time kernel panic (bt_poll asserting on a corrupt index); BT-only now boots+advertises clean. STILL BLOCKED on the 2 h gate by a remaining coex fault: under WiFi+BT concurrency a corrupt-stack/PC memory fault occurs (z_main_stack) and the BLE link is marginal — deeper than the ring-index assert. |
...
========================== DONE (2026-06-20) ==========================
W6 VERIFIED — the loop's stop condition is MET. 2 h continuous WiFi+BLE
coexistence under moderate load, 0 faults (71021 notif / 7200 s, PASS):
docs/artifacts/w6_soak_2h_clean_20260620.log. All of W0-W6 VERIFIED on the
Pico 2 W. The four durable fixes are in patches/ (see patches/README.md).
Optional remaining: W7 (flip the default transport to WHD). Final summary at
the bottom of this section / the commit log.
=======================================================================

(historical) NEXT: W6 §2.7 FIX DEMONSTRATED under a sustained connection; one
